...
- `normalizers/` – relocated legacy normalizer implementations (rigid + VoxelMorph engines, registration pipelines) that power `SpatialNormalizationService`.
- `preprocessing/` – reusable image preprocessing utilities shared by the normalizers.
...
- `SpatialNormalizationService` – Wraps the existing `RigidVoxelMorphNormalizer` behavior. It interprets `SpatialNormalizationRequest` flags (skip, manual FOV, iterative rigid) and directly constructs the normalizer as needed.
...
- `FileService` – Simple adapter around `Common::nifti::saveImage` so persistence can be mocked or replaced later.

## Metric modules and CLI integration
- Every metric family supplies both a CLI (`IMetricCLI`) and logic (`IMetricLogic`).
  - `metrics/suvr` implements `suvr`. Its logic (`SUVrLogic`) validates mask parameters and uses `SUVrCalculator` to produce `MetricResult` entries.
  - `metrics/centiloid` implements `centiloid`. It reuses the SUVR-derived CLI arguments, creates a `DCCCcore`, and prints tracer values (optionally exposing the intermediate SUVr).
  - `metrics/centaurz` implements `centaurz`. It follows the same CLI contract as the other SUVr-derived metrics, wraps `CenTauRzCalculator`, and reports the tracer-specific z-score conversions (with optional SUVr echo).
  - `metrics/fillstates` implements `fillstates`. It requires an explicit tracer flag, uses `FillStatesCalculator` to produce meta-ROI suprathreshold fractions, and optionally saves the fill-states mask map alongside the normalized output.
  - `metrics/adad` implements `adad`. It prepares an ADNI-style volume, runs the ONNX decoupler (single model or ensemble), and converts the ADAD score into tracer-specific values using the configuration-provided slopes/intercepts.
...
- Batch processing is currently available for `centiloid` and `centaurz`. Other refactor CLIs emit clear stderr messaging if `--batch` is supplied so downstream tooling understands the limitation.
...
localizer.exe suvr --input PET.nii --output PET_norm.nii `
    --config config.toml --voi-mask path/to/voi.nii --ref-mask path/to/ref.nii `
    --iterative --debug

# Centiloid prototype – reuses SUVr-derived arguments
localizer.exe centiloid --input PET.nii --output PET_centiloid.nii `
    --config config.toml --suvr

localizer.exe centaurz --input PET.nii --output PET_centaurz.nii `
    --config config.toml --suvr

localizer.exe centiloid --input PET_dir --output centiloid_outputs `
    --config config.toml --batch --skip-normalization

localizer.exe fillstates --input PET.nii --output PET_fillstates.nii `
    --config config.toml --tracer fbp

# ADAD prototype – selects the decoupling modality
localizer.exe adad --input PET.nii --output PET_adad.nii `
...
- Batch mode is currently implemented for `centiloid` and `centaurz` (directory-to-directory execution). Other refactor CLIs still emit clear stderr messaging if `--batch` is supplied so downstream tooling understands the limitation.
- Only the rigid VoxelMorph normalizer is available; additional normalizers would require extending `SpatialNormalizationService` to construct alternative implementations.
