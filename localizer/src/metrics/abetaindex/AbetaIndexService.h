...
class AbetaIndexService {
public:
    AbetaIndexService(ConfigurationPtr config,
                      std::shared_ptr<ISpatialNormalizationService> spatialService,
                      std::shared_ptr<IFileService> fileService);

    int run(AbetaIndexCLIOptions options, const std::string& fullCommand);

private:
    Metrics::MetricResult executeForImage(const AbetaIndexCLIOptions& options,
                                          const std::string& inputPath,
                                          const std::string& outputPath,
                                          const std::string& debugBasePath) const;
    int runSingle(const AbetaIndexCLIOptions& options, const std::string& fullCommand) const;
    int runBatch(const AbetaIndexCLIOptions& options, const std::string& fullCommand) const;
